    ...
    while True:
        ...
        for lane in lanes:
            if len(lane) >= 2:
                start_coord = lane[0]
                end_coord = lane[-1]
                #pts = np.array([start_coord, end_coord], dtype=np.int32).reshape((-1, 1, 2))
                #cv2.polylines(original_frame, pts , isClosed=False, color=(0,255,0), thickness=3)
                cv2.line(original_frame, start_coord, end_coord, (0, 255, 0), 3)
            #for coord in lane:
                #cv2.circle(original_frame, coord, radius, (0, 255, 0), -1)
                
        #sw 
        #cp = np.array(cumulative_points, dtype=np.int32).reshape((-1,1,2))
        #print(cp)
        #cv2.polylines(original_frame, cp, isClosed=False, color=(0,255,0), thickness=3)  
        #for cpi in cumulative_points :
        #    cv2.circle(original_frame, cpi, radius, (0, 255, 0), -1)      
